...
def describe_vault():
    def describe_init():
        def it_instantiates(mocker):
            ...
            obj = Vault(addr="foourl", kvv2_mount_point="foomountpoint", path="foopath")
    ...
    def describe_kubernetes_login():
        ...
        def it_calls_kubernetes_login_when_role_is_defined(mocker, tmp_path):
            ...
            spy.assert_called_once_with(role="foorole", jwt="footoken")
    ...
    def describe_get():
        def it_gets(mocker):
            ...
            assert (
                obj.get("fookey")
                == lib.vault.hvac.Client().secrets.kv.read_secret_version()["data"][
                    "data"
                ]["key"]
            )
            spy.assert_has_calls(
                [mocker.call(path="foopathfookey", mount_point="foomountpoint")]
            )
    ...
    def describe_list():
        def it_lists(mocker):
            ...
            spy.assert_has_calls(
                [mocker.call(path="foopath", mount_point="foomountpoint")]
            )
